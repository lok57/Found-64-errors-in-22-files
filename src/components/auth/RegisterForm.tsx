...
export default function RegisterForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (password !== confirmPassword) {
      toast.error('Passwords do not match'); // Use toast for error notification
      return;
    }
    ...
    try {
      await register(email, password, name); // Ensure register accepts name as the third argument
      navigate('/');
    } catch (error) {
      toast.error('Registration failed. Please try again.'); // Add toast error for feedback
    } finally {
      setLoading(false);
    }
  };
